...
export type BackendAppService = { backend: 'api' | 'renderer' | 'redirecter' | 'backup' };
export type FrontendAppService = { frontend: 'dashboard' | 'registration' | 'webshop' | 'calculator' | 'mobile' };
...
function buildDevelopmentDomains(context: CliContext) {
    ...
    const labels = [prefix, context.env === 'stamhoofd' ? '' : context.env, domain].filter(label => label.length > 0);
...
function frontendPort(service: FrontendAppService['frontend'], ports: ReturnType<typeof buildPorts>): number | undefined {
    if (service === FrontendApp.Dashboard) {
        return ports.dashboard;
    }
    if (service === FrontendApp.Registration) {
        return ports.registration;
    }
    if (service === FrontendApp.Webshop) {
...
function backendPort(service: BackendAppService['backend'], ports: ReturnType<typeof buildPorts>): number {
    if (service === BackendApp.Renderer) {
