...
describe('buildInstance', () => {
    it('keeps the default Stamhoofd main instance on base ports', () => {
        const instance = buildInstance({ env: 'stamhoofd', workspace: 'main', branch: 'main', primary: true });
    ...
    it('adds workspace and branch isolation for secondary instances', () => {
        const instance = buildInstance({ env: 'keeo', workspace: 'Andreas Repo', branch: 'feature/payments', primary: false });

        expect(instance.name).toBe('stamhoofd-andreas-repo-feature-payments-keeo');
        expect(instance.prefix).toBe('andreas-repo-feature-payments');
    ...
    it('allows explicit name and port overrides', () => {
        ...
        const instance = buildInstance({ env: 'stamhoofd', workspace: 'main', branch: 'main', primary: true, overrideName: 'Custom Name' });
