...
export function buildInstance(options: { env: string; workspace: string; branch: string | null; primary: boolean; overrideName?: string }) {
    const envSlug = slug(options.env) || 'stamhoofd';
    const branchSlug = options.branch ? slug(options.branch) : '';
    const inferredPrefix = options.primary ? '' : slug([options.workspace, branchSlug].filter(Boolean).join('-'));
    ...
    const defaultOffset = primary && envSlug === 'stamhoofd' ? 0 : ((checksum(`${options.workspace}:${branchSlug}:${envSlug}`) % 50) + 1) * 100;
