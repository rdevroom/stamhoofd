...
export async function runDev(context: CliContext, target: 'all' | 'instance' | 'backend' | 'frontend', options: { services: boolean; stripe: boolean; open?: boolean }): Promise<void> {
    ...
    try {
        ...
        const child = spawn('pnpm', ['exec', 'concurrently', '-r', sharedBuildWatchCommand(), `${sharedBuildReadyCommand()} && ${command}`], {
            cwd: context.rootDir,
            env: {
                ...process.env,
        ...
        child.stdout?.on('data', chunk => output.write(chunk, 'stdout'));
        child.stderr?.on('data', chunk => output.write(chunk, 'stderr'));
...
function formatDomainLabel(domain: string, env: string): string {
    ...
    const leading = segments.slice(0, highlightIndex).join('.');
    const between = environmentSegment ? segments.slice(highlightIndex + 1, environmentIndex).join('.') : '';
    const trailing = environmentSegment ? segments.slice(environmentIndex + 1).join('.') : segments.slice(highlightIndex + 1).join('.');
    const parts = [prefix];
...
function commandForTarget(target: 'all' | 'instance' | 'backend' | 'frontend'): string {
    if (target === 'backend') {
        return 'pnpm --filter @stamhoofd/backend --filter @stamhoofd/backend-renderer -r --parallel --stream run dev';
    }
    if (target === 'frontend') {
        return 'pnpm --filter @stamhoofd/dashboard --filter @stamhoofd/registration --filter @stamhoofd/webshop -r --parallel --stream run dev';
    }
    return 'pnpm --filter @stamhoofd/backend --filter @stamhoofd/backend-renderer --filter @stamhoofd/dashboard --filter @stamhoofd/registration --filter @stamhoofd/webshop -r --parallel --stream run dev';
}
