...
describe('runDev', () => {
    ...
    it('starts app processes through pnpm and waits for shared CLI and locale builds', async () => {
        ...
        expect(spawn).toHaveBeenCalledWith('pnpm', [
            ...
            'concurrently',
            '-r',
            expect.stringMatching(/rm -f \.development\/cli\/generated\/shared-build-\d+\.ready.+pnpm --dir shared\/cli -s build.+touch \.development\/cli\/generated\/shared-build-\d+\.ready/),
            expect.stringMatching(/wait-on \.development\/cli\/generated\/shared-build-\d+\.ready shared\/cli\/dist\/index\.js shared\/locales\/dist\/index\.d\.ts && pnpm --filter @stamhoofd\/backend --filter @stamhoofd\/backend-renderer --filter @stamhoofd\/dashboard --filter @stamhoofd\/registration --filter @stamhoofd\/webshop -r --parallel --stream run dev/),
