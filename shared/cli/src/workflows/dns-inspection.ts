...
import { defaultDomain, localIpv4Host, localhostPort } from '../config/shared-service-config.js';
...
import { corednsService, CorednsService, type CorednsRecord } from '../services/definitions/coredns-service.js';
...
export async function inspectDns(context: CliContext, options: { stopAfterOsConfigFailure?: boolean } = {}): Promise<DnsInspection> {
    ...
    const osConfigContent = osConfig.path ? await readFileIfExists(osConfig.path) : undefined;
    const osConfigMatches = osConfig.content !== undefined && osConfigContent === osConfig.content;
    const osCheck = osConfigCheck(domain, profile, osConfig.path, osConfigMatches, osConfigContent);
    if (options.stopAfterOsConfigFailure && !osCheck.ok) {
        return buildInspection({ domain, query, profile, osConfig, osConfigContent, osConfigMatches, corednsRunning: false, directCorednsAddresses: [], systemResolverAddresses: [], checks: [osCheck] });
    }
    ...
    const directCorednsAddresses = corednsRunning ? await directCorednsLookup(query, profile) : [];
    ...
    return buildInspection({
        ...
        checks: [
            osCheck,
            {
                ...
                ok: corednsRunning,
                details: corednsRunning ? 'stamhoofd-coredns is running' : 'stamhoofd-coredns is not running',
            ...
            },
            {
                ...
                ok: directCorednsOk,
                details: directCorednsOk ? `${query} -> ${localIpv4Host} via ${localhostPort(profile.corednsHostPort)}` : `${query} did not resolve directly through CoreDNS at ${localhostPort(profile.corednsHostPort)}`,
            ...
            },
            {
                ...
                ok: systemResolverOk,
                details: systemResolverOk ? `${query} -> ${localIpv4Host}` : `${query} did not resolve through system DNS`,
...
function buildInspection(options: { domain: string; query: string; profile: SharedServiceProfile; osConfig: { path?: string; content?: string }; osConfigContent?: string; osConfigMatches: boolean; corednsRunning: boolean; directCorednsAddresses: string[]; systemResolverAddresses: string[]; checks: DnsCheck[] }): DnsInspection {
    return {
...
export function expectedOsDnsConfig(domain: string, profile: SharedServiceProfile): { path?: string; content?: string } {
    if (profile.dnsSetupKind === SharedServiceDnsSetupKind.MacosResolver) {
        return { path: macosResolverPath(domain), content: macosResolverContent() };
    }
    if (profile.dnsSetupKind === SharedServiceDnsSetupKind.SystemdResolved) {
        return { path: '/run/systemd/resolved.conf.d/stamhoofd.conf', content: systemdResolvedContent(domain, profile) };
    }
    return {};
}

export function systemdResolvedContent(domain: string, profile: SharedServiceProfile): string {
...
async function directCorednsLookup(query: string, profile: SharedServiceProfile): Promise<string[]> {
...
async function systemResolverLookup(query: string, profile: SharedServiceProfile): Promise<string[]> {
    if (profile.dnsSetupKind === SharedServiceDnsSetupKind.SystemdResolved) {
        const result = await run('resolvectl', ['query', query], { capture: true, allowFailure: true });
...
function osConfigCheck(domain: string, profile: SharedServiceProfile, configPath: string | undefined, matches: boolean, currentContent: string | undefined): DnsCheck {
    if (!configPath) {
        return { key: 'os-config', label: 'System DNS config', ok: false, details: `Automatic DNS setup is not supported on ${profile.platform}`, fix: 'Configure DNS manually' };
    }
    if (matches) {
        return {
            key: 'os-config',
    ...
    return {
        ...
        label: 'System DNS config',
        ok: false,
        details: currentContent === undefined ? `${configPath} is missing` : `${configPath} exists with unexpected contents`,
