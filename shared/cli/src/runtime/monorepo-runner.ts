...
export async function buildShared(context: CliContext): Promise<void> {
    ...
    for (const packagePath of globalSharedPackages) {
        await run('pnpm', ['--dir', packagePath, '-s', 'build'], { cwd: context.rootDir, verbose: context.verbose });
    }
...
export async function testUnit(context: CliContext, ci: boolean): Promise<void> {
    ...
    try {
        await run('pnpm', ['exec', 'lerna', 'run', 'test', '--ignore', '@stamhoofd/playwright', '--ignore', '@stamhoofd/dashboard'], { cwd: context.rootDir, env: { NX_DAEMON: 'false', CI: ci ? 'true' : undefined, DB_PORT: dbPort }, verbose: context.verbose });
    }
    finally {
...
export async function testE2e(context: CliContext, options: { ci: boolean; ui: boolean }): Promise<void> {
    await buildShared(context);
    await run('pnpm', ['--dir', 'backend/app/api', '-s', 'build:playwright:pre'], { cwd: context.rootDir, verbose: context.verbose });
    await run('pnpm', ['--dir', 'tests/playwright', '-s', 'test', ...(options.ui ? ['--ui'] : [])], { cwd: context.rootDir, env: { NX_DAEMON: 'false', CI: options.ci ? 'true' : undefined }, verbose: context.verbose });
}
...
export function sharedBuildWatchCommand(): string {
    const command = [...globalSharedPackages, ...backendSharedPackages]
        .map(packagePath => `pnpm --dir ${packagePath} -s build`)
        .join(' && ');
